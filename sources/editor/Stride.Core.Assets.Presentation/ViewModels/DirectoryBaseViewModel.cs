...
public abstract class DirectoryBaseViewModel : SessionObjectViewModel, IChildViewModel, IAddChildViewModel
{
        ...
        : base(session)
    {
        ...
        DependentProperties.Add(nameof(Parent), [nameof(Path), nameof(Package), nameof(Root)]);
    ...
    public override IEnumerable<IDirtiable> Dirtiables => Parent is not null ? base.Dirtiables.Concat(Parent.Dirtiables) : base.Dirtiables;
    ...
    {
        if (children.Any(x => x is IIsEditableViewModel { IsEditable: false }))
        {
        ...
        foreach (var child in children)
        {
            switch (child)
            {
                case MountPointViewModel:
                    //message = DragDropBehavior.InvalidDropAreaMessage; // FIXME xplat-editor
                    message = Tr._p("Message", "Can't drop here");
                    return false;
                case DirectoryViewModel directory when directory == this:
                    message = "Can't drop a folder on itself or its children";
                    return false;
                case DirectoryViewModel directory when directory.Root.GetType() != Root.GetType():
                    message = "Incompatible folder";
                    return false;
                case DirectoryViewModel directory when SubDirectories.Any(x => x.Name == directory.Name):
                    message = $"Directory {((this is DirectoryViewModel) ? Path : Name)} already contains a subfolder named {directory.Name}";
                    return false;
                case DirectoryViewModel directory when children.OfType<DirectoryViewModel>().Any(x => x != child && string.Compare(x.Name, directory.Name, StringComparison.OrdinalIgnoreCase) == 0):
                ...
                case AssetViewModel { IsLocked: true } asset:
                    message = $"Asset {asset.Url} can't be moved";
                    return false;
                case AssetViewModel asset when Assets.Any(x => string.Compare(x.Name, asset.Name, StringComparison.OrdinalIgnoreCase) == 0):
                    message = $"{(!string.IsNullOrEmpty(Path) ? $"Directory {Path}" : "This directory")} already contains an asset named {asset.Name}";
                    return false;
                case AssetViewModel asset when children.OfType<AssetViewModel>().Any(x => x != child && string.Compare(x.Name, asset.Name, StringComparison.OrdinalIgnoreCase) == 0):
                    message = "Can't move assets with duplicate names";
                    return false;
                case AssetViewModel asset when asset.Directory.Root.GetType() != Root.GetType():
                    message = "Incompatible folder";
                    return false;
                case AssetViewModel:
                    break;
