...
{
    private string name;
    private DirectoryBaseViewModel parent = null!;
        ...
        : base(parent.Package.Session)
    {
        ...
        if (!IsValidName(name, out var errorMessage)) throw new ArgumentException(errorMessage);
    ...
    /// <inheritdoc/>
    public ThumbnailData ThumbnailData => folderThumbnail ??= GetFolderThumbnail(Dispatcher);
    ...
    {
        if (IsValidName(newName, out var error) == false)
        {
            ServiceProvider.Get<IDialogService>()
                .MessageBoxAsync(string.Format(Tr._p("Message", "Unable to rename folder. {0}"), error), MessageBoxButton.OK, MessageBoxImage.Error)
                .Wait(); // FIXME dangerous
            //ServiceProvider.Get<IDialogService2>().BlockingMessageBox(string.Format(Tr._p("Message", "Unable to rename folder. {0}"), error), MessageBoxButton.OK, MessageBoxImage.Error); // FIXME xplat-editor
            return;
        }
    ...
    protected override bool IsValidName(string value, out string? error)
    {
        ...
        return IsPathAcceptedByFilesystem(UPath.Combine<UDirectory>(Package.PackagePath.GetFullDirectory(), value), out error);
    }
