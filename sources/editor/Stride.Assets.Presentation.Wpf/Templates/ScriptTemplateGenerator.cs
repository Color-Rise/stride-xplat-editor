        ...
        protected override async Task<bool> PrepareAssetCreation(AssetTemplateGeneratorParameters parameters)
        {
            if (!parameters.Unattended)
            {
                ...
                if (ask)
                {
                    ...
                    var result = await EditorViewModel.Instance.ServiceProvider.Get<IDialogService2>().CheckedMessageBox(message, false, checkedMessage, buttons, MessageBoxImage.Question);
