    ...
    public abstract class UIEditorBaseViewModel : AssetCompositeHierarchyEditorViewModel<UIElementDesign, UIElement, UIElementViewModel>, IAddChildViewModel
    {
        ...
        protected override async Task Delete()
        {
            ...
            if (ask)
            {
                ...
                var result = await ServiceProvider.Get<IDialogService2>().CheckedMessageBox(confirmMessage, false, DialogHelper.DontAskAgain, buttons, MessageBoxImage.Question);
                if (result.Result != 1)
                    return;
