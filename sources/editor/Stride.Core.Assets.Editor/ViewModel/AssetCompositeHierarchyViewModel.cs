...
{
    protected AssetCompositeHierarchyViewModel([NotNull] AssetViewModelConstructionParameters parameters)
        : base(parameters)
    {
    ...
    public override AssetCompositeHierarchyPropertyGraph<TAssetPartDesign, TAssetPart> PropertyGraph => (AssetCompositeHierarchyPropertyGraph<TAssetPartDesign, TAssetPart>)base.PropertyGraph;
    ...
    public IEnumerable<AssetCompositeHierarchyViewModel<TAssetPartDesign, TAssetPart>> GatherAllBasePartAssets()
    {
        ...
        while (assetToProcess.Count > 0)
        {
            ...
            foreach (var basePartAsset in asset.PropertyGraph.GetBasePartAssets())
            {
    ...
    protected override bool ShouldConstructPropertyMember(IMemberNode member)
    {
        ...
        if (PropertyGraph.IsChildPartReference(member, NodeIndex.Empty))
            return false;
    ...
    protected override bool ShouldConstructPropertyItem(IObjectNode collection, NodeIndex index)
    {
        ...
        if (PropertyGraph.IsChildPartReference(collection, index))
            return false;
    ...
    private void AssetsDeleted(object sender, [NotNull] NotifyCollectionChangedEventArgs e)
    {
        ...
        {
            foreach (var assetPropertyGraph in e.NewItems.OfType<AssetCompositeHierarchyViewModel<TAssetPartDesign, TAssetPart>>().Select(x => ((AssetViewModel)x).PropertyGraph).OfType<AssetCompositeHierarchyPropertyGraph<TAssetPartDesign, TAssetPart>>())
            {
                var instanceIds = PropertyGraph.GetInstanceIds(assetPropertyGraph);
                PropertyGraph?.BreakBasePartLinks(Asset.Hierarchy.Parts.Values.Where(x => x.Base != null && instanceIds.Contains(x.Base.InstanceId)));
            }
        }
    }
    ...
    private void BaseChanged(object sender, MemberNodeChangeEventArgs e)
    {
        ...
        if (baseInfo == null)
        {
            PropertyGraph.RefreshBase((IAssetNode)part, null);
        }
        else
        {
            ...
            if (baseAsset != null && baseAsset.Asset.Hierarchy.Parts.TryGetValue(baseInfo.BasePartId, out basePart))
            {
                ...
                PropertyGraph.RefreshBase((IAssetNode)part, basePartNode);
            }
        }
    }
}
