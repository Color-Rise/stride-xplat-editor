...
public class NodeViewModel : DispatcherViewModel, IDynamicMetaObjectProvider
{
    ...
    /// </summary>
    public string DisplayPath { get { if (Parent is null) return string.Empty; var parentPath = Parent.DisplayPath; return parentPath != string.Empty ? parentPath + '.' + DisplayName : DisplayName; } }
    ...
    {
        if (initializingChildren is null)
        {
    ...
    private static int CompareChildren(NodeViewModel? a, NodeViewModel? b)
    {
        ...
        if (a?.Order is null && b?.Order is null)
        {
